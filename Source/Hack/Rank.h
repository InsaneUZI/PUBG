...
public:
    ...
    void concurrentRequests(const std::vector<std::string>& names, std::unordered_map<std::string, bool>& QuerySuccess, std::unordered_map<std::string, int>& QueryCount, std::unordered_map<std::string, PlayerRankList>& all_rank) {
        ...
        Data::SetPlayerRankLists(all_rank);
    }
    ...
    static void Update()
    {
        ...
        std::unordered_set<std::string> PlayerNameList;
        std::unordered_map<std::string, PlayerRankList> all_rank;
        ...
        while (true)
        {
            ...
            if (!GameData.Config.ESP.Tier && !GameData.Config.ESP.RankPoint && !GameData.Config.ESP.KDA || GameData.Config.PlayerList.RankMode ==0 )
            {
            ...
            for (auto& player : PlayerNameList)
            {
                if (QuerySuccess.find(player) == QuerySuccess.end())
                {
                    QuerySuccess[player] = false;
                }

                if (QueryCount.find(player) == QueryCount.end())
                {
                    QueryCount[player] = 0;
                }

                if (QuerySuccess[player])
                {
                    continue;
                }
            ...
            {
                std::cout << pPlayers[0];
                rank.concurrentRequests(pPlayers, QuerySuccess, QueryCount, all_rank);
            }
