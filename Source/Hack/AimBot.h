...
public:
    ...
    static FVector GetPredicted(const FVector& GunLocation, FVector TargetPos, FVector TargetVelocity, FWeaponTrajectoryConfig TrajectoryConfig) {
        ...
        float Drop = 0.5f * Gravity * TimeToReach * TimeToReach * 45.0f;
        ...
        float BallisticDropScale, float BDS, float SimulationSubstepTime, float VDragCoefficient, FRichCurve RichCurve, int KeysNum, std::vector<FRichCurveKey> Keys)
    {
        ...
        //子弹投掷                                              旅行时间
        float BulletDrop = 0.5f * TrajectoryGravityZ * TravelTime * TravelTime * 89.0f;
    ...
    static void Run()
    {
        ...
        while (true)
        {
            ...
            Utils::Log(1, "IsFirstKey: %d", IsFirstKey);
            Utils::Log(1, "IsSecondKey: %d", IsSecondKey);
            Utils::Log(1, "IsGroggyKey: %d", IsGroggyKey);
            Utils::Log(1, "IsWheelKey: %d", IsWheelKey);
            ...
            if (CanAim || (GameData.LocalPlayerInfo.IsScoping && GameData.Config.AimBot.ShowPoint))
            {
                ...
                if (Utils::ValidPtr(LastCurrentWeapon) || LastCurrentWeapon != GameData.LocalPlayerInfo.CurrentWeapon)
                {
                    ...
                    Utils::Log(1, "CurrentWeaponData.Mesh3P: %p", CurrentWeaponData.Mesh3P);
                    ...
                    Utils::Log(1, "CurrentWeaponData.TrajectoryConfigs.InitialSpeed: %f", CurrentWeaponData.TrajectoryConfigs.InitialSpeed);
                ...
                if (!FistAim)
                {
                    ...
                    Utils::Log(1, "InputAxisProperties: %p", InputAxisProperties);
                ...
                {
                    Utils::Log(1, "GameData.AimBot.Target: %d", GameData.AimBot.Target);
                    StopAiming();
                    continue;
                }
                ...
                {
                    Utils::Log(1, "StopAiming InScopePoint: %d", InScopePoint);
                    ...
                    {
                        LineTraceHook::StartHook();
                        EBoneIndex VisibilityBoneIndex;

                        if (LineTraceHook::GetLocation(Player, (EBoneIndex)AimBone, &VisibilityBoneIndex))
                        {
                    ...
                    Utils::Log(1, "StopAiming IsWheelKey: %d", IsWheelKey);
                    if (!IsWheelKey)
                    {
                        StopAiming();
                        continue;
                ...
                mem.ExecuteReadScatter(hScatter);
                Utils::Log(1, "IsReloading: %d", IsReloading);
                Utils::Log(1, "IsReloading: %d", (Config.NoBulletNotAim&& BulletNumber == 0));
                ...
                Utils::Log(1, "TargetDistance: %d", TargetDistance);
                Utils::Log(1, "Config.AimDistance: %d", Config.AimDistance);
                ...
                float BulletDrop = 0;
                float TravelTime = 0;
                float BallisticDragScale = 1.05;
                float BallisticDropScale = 0.64;
                ...
                FVector PredictedPos = FVector(TargetPos.X, TargetPos.Y, TargetPos.Z + Abs(BulletDrop)) + TargetVelocity * (TravelTime / 1.f);
                ...
                if (Config.LineTraceSingle)
                {
                    LineTraceHook::EndHook();
                }
            }
