...
namespace MenuPlayerLists
{
    ...
    inline void Render()
    {
        ...
        ImGui::SetNextWindowSize({ 990, 600 });
        ImGui::SetNextWindowPos(ImGui::GetMainViewport()->Pos + ImVec2{ 100, 50 });
        ...
        if (GameData.Config.Window.Players && ImGui::Begin(U8("玩家列表"), &GameData.Config.Window.Players, ImGuiWindowFlags_NoBackground | ImGuiWindowFlags_NoDecoration | ImGuiWindowFlags_NoDocking | ImGuiWindowFlags_NoSavedSettings | ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoResize))
        {
            ...
            ImGui::GetWindowDrawList()->AddRectFilled(Pos, ImVec2(Pos.x + 990, Pos.y + 600), ImGui::GetColorU32(winbg_color), 10.f);
            
            ImGui::SetCursorPos(ImVec2(40.f + Spacing.x / 2, 40.f + 40));
            ...
            if (ImGui::NewBeginCombo(U8("##名单类型"), filterPlayerList[filterPlayerListIndex], 0, true))
            {
            ...
            if (ImGui::NewBeginCombo(U8("##玩家类型"), filterPlayerType[filterPlayerTypeIndex], 0, true)) 
            {
            ...
            if (ImGui::NewBeginCombo(U8("##战绩数据"), items[GameData.Config.PlayerList.RankMode], 0, true)) {
            ...
            ImGui::MyInputTextEx(U8("赛季选择"), "31", GameData.Config.ESP.RankSize, IM_ARRAYSIZE(GameData.Config.ESP.RankSize), ImVec2(40, 30), NULL);
            ImGui::PopStyleColor();

            ImGui::SameLine();
            ImGui::PushStyleColor(ImGuiCol_Text, ImVec4{ 0, 0, 0, 1.f });

            ImGui::MyInputTextEx(U8("搜索玩家"), "", filterPlayerName, IM_ARRAYSIZE(filterPlayerName), ImVec2(200, 30), NULL);
            ImGui::PopStyleColor();
            ...
            ImGui::SetCursorPos(ImVec2(40.f + Spacing.x / 2, 40.f + 90));
            if (ImGui::BeginTable(U8("\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t玩家列表"), 11, flags, ImVec2(900, 400.f)))
            {
