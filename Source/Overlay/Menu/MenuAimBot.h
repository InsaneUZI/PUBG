...
public:
    static void Setting(std::string type)
    {
        ...
        {
            ImGui::Keybind(U8("锁甲热键"), &Config.First.Key, true);
            ImGui::Keybind(U8("锁头热键"), &Config.Second.Key, true);
            ImGui::NewCheckbox(U8("热键合并"), &Config.HotkeyMerge);
            ImGui::Keybind(U8("倒热键地"), &Config.Groggy.Key, true);
            const char* aim_mode[] = { U8("范围优先"),U8("距离优先") };
            ImGui::NewCombo(U8("自瞄模式"), &Config.AimMode, aim_mode, IM_ARRAYSIZE(aim_mode));
        ...
        ImGui::BeginChild("O", U8("选项"), ImVec2(276, 240));
        {
            ...
            /*if(!Config.LineTraceSingle) ImGui::NewCheckbox(U8("漏那变色"), &Config.VisibleCheck);*/
            ImGui::NewCheckbox(U8("自瞄开关"), &Config.enable);

            ImGui::NewCheckbox(U8("自由拉枪"), &Config.FOVenable);

            ImGui::NewCheckbox(U8("掩体不瞄"), &Config.VisibleCheck);

            ImGui::NewCheckbox(U8("开镜检测"), &Config.bIsScoping_CP);
        ...
        ImGui::BeginGroup();
        {
            ...
            if (SubSelectedTab == 0)
            {
                ImGui::BeginChild("O", U8("自瞄配置"), ImVec2(276, 310));
                {
                    ...
                    ImGui::Keybind(U8("自瞄配置 一"), &GameData.Config.AimBot.Configs[0].Key, true);
                    ImGui::Keybind(U8("自瞄配置 二"), &GameData.Config.AimBot.Configs[1].Key, true);
                }
                ImGui::EndChild();
                ...
                if (GameData.Config.AimBot.Mode == 0)
                {
                    ...
                    ImGui::BeginChild("O", U8("硬件设备"), ImVec2(276, SizeY));
                    {
                        ...
                        if (GameData.Config.AimBot.Controller == 1)
                        {
                            ...
                            ImGui::SliderInt(U8("自瞄延时"), &GameData.Config.AimBot.ms, 1, 100);
                        }
