...
namespace RenderHelper {
	...
	inline void Image(const ImTextureID& Texture, ImVec2 Pos, ImVec2 Size, bool UseColorOverlay = false, const ImU32& Color = IM_COL32(255, 255, 255, 255), bool isWindowDrawList = true)
	{
		...
		if (UseColorOverlay)
		{
			DrawList->AddImage(Texture, Pos, Size, ImVec2(0, 0), ImVec2(1, 1), Color);
		}
		else {
			DrawList->AddImage(Texture, Pos, Size);
		}
	}
	...
	inline void DrawRadarPlayerCircleWithText(const char* text, ImU32 circle_col, float radius, ImVec2 position, float angle, ECharacterIconType RadarState) {
		...
		if (GImGuiTextureMap[IconUrl].Width > 0) {
			...
			RenderHelper::AddImageRotated((ImTextureID)GImGuiTextureMap[IconUrl].Texture, IconPos, ImVec2(IconSize, IconSize), angle);
		}
		...
		if (RadarState != ECharacterIconType::Normal)
		{
			...
			if (GImGuiTextureMap[RadarStateIconUrl].Width > 0) {
				...
				RenderHelper::AddImageRotated((ImTextureID)GImGuiTextureMap[IconUrl].Texture, IconPos, ImVec2(IconSize, IconSize), 0);
			}
		}
		else {
	...
	void DrawRay(ImDrawList* DrawList, const ImVec2& start, float angle, ImU32 color, float thickness, float length = 1000.0f)
	{
		//ImDrawList* drawList = ImGui::GetWindowDrawList();
		float radians = angle * (3.14159265f / 180.0f);
		ImVec2 direction(cos(radians), sin(radians));
		ImVec2 end = ImVec2(start.x + direction.x * length, start.y + direction.y * length);
		DrawList->AddLine(start, end, color, thickness);
	}

	//2D
	inline void Draw2DRadarPlayerCircleWithText(const char* text, ImU32 circle_col, float radius, ImVec2 position, float angle, ImDrawList* DrawList, bool isMe, float Yaw, bool isAimMe) {
		...
		ImVec2 CircleFilledPos = position;
		...
		if (GImGuiTextureMap[IconUrl].Width > 0) {
			...
			float IconSize = radius * 10.3f;
			RenderHelper::AddImageRotated((ImTextureID)GImGuiTextureMap[IconUrl].Texture, IconPos, ImVec2(IconSize, IconSize), angle, false);
		}

		if (isMe)
		{
			DrawRay(DrawList, AimPos, Yaw, IM_COL32(0, 255, 0, 255), 2.0f);
		}

		if (isAimMe)
		{
			DrawRay(DrawList, AimPos, Yaw, IM_COL32(255, 0, 0, 255), 2.0f);
		}

		DrawList->AddCircleFilled(CircleFilledPos, radius, circle_col, radius * 2);
		DrawList->AddText(ImGui::GetFont(), fontSize, ImVec2(textPos.x - 1, textPos.y + 1), ImColor(45, 45, 45, 220), text);
		DrawList->AddText(ImGui::GetFont(), fontSize, ImVec2(textPos.x - 1, textPos.y - 1), ImColor(45, 45, 45, 220), text);
		DrawList->AddText(ImGui::GetFont(), fontSize, ImVec2(textPos.x + 1, textPos.y + 1), ImColor(45, 45, 45, 220), text);
		DrawList->AddText(ImGui::GetFont(), fontSize, ImVec2(textPos.x + 1, textPos.y - 1), ImColor(45, 45, 45, 220), text);
		DrawList->AddText(ImGui::GetFont(), fontSize, textPos, IM_COL32_WHITE, text);
	}
