...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    static std::map<ImGuiID, c_button> anim;
    auto it_anim = anim.find(id);

    if (it_anim == anim.end())
    {
        anim.insert({ id, c_button() });
    ...
    it_anim->second.background = ImLerp(it_anim->second.background, g.ActiveId == g.LastItemData.ID ? bg_color : ImColor(bg_color.x, bg_color.y, bg_color.z, 0.5f).Value, anim_speed);

    // Render
    RenderNavHighlight(bb, id);
    RenderFrame(bb.Min, bb.Max, GetColorU32(it_anim->second.background), true, style.FrameRounding);
...
bool ImGui::SubTab(const char* label, int* v, int number)
{
    ...
    const ImVec2 pos = window->DC.CursorPos;
    const ImRect total_bb(pos, pos + ImVec2(40 + CalcTextSize(label).x, 30));
    ...
    it_anim->second.frame_col = ImLerp(it_anim->second.frame_col, *v == number ? tab_color : ImColor(tab_color.x, tab_color.y, tab_color.z, 0.f).Value, anim_speed);
    it_anim->second.text_col = ImLerp(it_anim->second.text_col, *v == number ? text_color[0] : text_color[1], anim_speed);
    it_anim->second.line_col = ImLerp(it_anim->second.line_col, *v == number ? text_color[0] : ImColor(text_color[0].x, text_color[0].y, text_color[0].z, 0.f).Value, anim_speed);

    window->DrawList->AddRectFilled(total_bb.Min, total_bb.Max, GetColorU32(it_anim->second.frame_col), style.FrameRounding);

    window->DrawList->AddText(center_text(total_bb.Min, total_bb.Max, label) + ImVec2(5, 0), GetColorU32(it_anim->second.text_col), label);

    window->DrawList->AddRectFilled(total_bb.Min + ImVec2(10, 7), total_bb.Max - ImVec2(total_bb.GetSize().x - 12, 8), GetColorU32(it_anim->second.line_col), 360.f, ImDrawFlags_RoundCornersTop);
...
bool ImGui::Tab(const char* label, const char* icon, int* v, int number)
{
    ...
    const float square_sz = GetFrameHeight();
    const ImVec2 pos = window->DC.CursorPos;
    const ImRect total_bb(pos, pos + ImVec2(170, 30));
    ...
    it_anim->second.frame_col = ImLerp(it_anim->second.frame_col, *v == number ? tab_color : ImColor(tab_color.x, tab_color.y, tab_color.z, 0.f).Value, anim_speed);
    it_anim->second.text_col = ImLerp(it_anim->second.text_col, *v == number ? text_color[0] : text_color[1], anim_speed);
    it_anim->second.line_col = ImLerp(it_anim->second.line_col, *v == number ? main_color : ImColor(0, 0, 0, 0).Value, anim_speed);

    window->DrawList->AddRectFilled(total_bb.Min - ImVec2(0, 10), total_bb.Max + ImVec2(0, 20), GetColorU32(it_anim->second.frame_col), style.FrameRounding);
    PushFont(icon_font);
    window->DrawList->AddText(ImVec2(total_bb.Min.x + 22.5f, center_text(total_bb.Min, total_bb.Max, label).y + 10.f), GetColorU32(it_anim->second.text_col), icon);
    PopFont();

    PushFont(PlayersFont);
    ...
    window->DrawList->AddRectFilled(total_bb.Min + ImVec2(10, 0), total_bb.Max - ImVec2(total_bb.GetSize().x - 12, 0), GetColorU32(it_anim->second.line_col), 360.f);
...
bool ImGui::Keybind(const char* label, int* key)
{
    ...
    if (label_str.find("##keybind") == std::string::npos)
        window->DrawList->AddText(ImVec2(total_bb.Min.x, center_text(total_bb.Min, total_bb.Max, label).y), GetColorU32(text_color[0]), label);

    PushFont(semibold_font);
    ...
    if (*key > 0 && !it_anim->second.mode)
    {
        ...
        if (*key < 250)
            it_anim->second.key_name = keys_name[*key];
    ...
    {
        PushFont(icon_font);
        it_anim->second.key_size = ImLerp(it_anim->second.key_size, CalcTextSize("k"), anim_speed);
        window->DrawList->AddText(ImVec2(total_bb.Max.x - it_anim->second.key_size.x, center_text(total_bb.Min, total_bb.Max, "k").y), GetColorU32(it_anim->second.key_color), "k");
        PopFont();
    }
    return pressed;
}
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    const ImVec2 check_pos = pos + ImVec2(8, 8);
    ...
    it_anim->second.frame_offset = ImLerp(it_anim->second.frame_offset, pressed ? ImVec2(12, 12) : *v ? ImVec2(8, 8) : ImVec2(0, 0), anim_speed);
    it_anim->second.frame_color = ImLerp(it_anim->second.frame_color, *v ? main_color : ImVec4(0, 0, 0, 0), anim_speed * 2);
    it_anim->second.mark_color = ImLerp(it_anim->second.mark_color, *v ? check_mark_col : ImVec4(0, 0, 0, 0), anim_speed * 2);
    it_anim->second.text_color = ImLerp(it_anim->second.text_color, *v ? text_color[0] : text_color[1], anim_speed * 2);

    window->DrawList->AddRect(check_pos - ImVec2(8, 8), check_pos + ImVec2(8, 8), GetColorU32(text_color[1]), style.FrameRounding, 0, 1.9f);

    window->DrawList->AddRectFilled(check_pos - it_anim->second.frame_offset, check_pos + it_anim->second.frame_offset, GetColorU32(it_anim->second.frame_color), style.FrameRounding);

    RenderCheckMark(window->DrawList, check_pos - it_anim->second.frame_offset / 2, GetColorU32(it_anim->second.mark_color), it_anim->second.frame_offset.x);

    window->DrawList->AddText(ImVec2(total_bb.Min.x + 22, center_text(total_bb.Min, total_bb.Max, label).y), GetColorU32(it_anim->second.text_color), label);
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags, int value, bool hide_lable)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = GetCurrentWindow();
    ...
    const ImRect total_bb(pos, pos + frame_size);
    const ImRect rect_bb(total_bb.Min + (hide_lable ? ImVec2(0, -2) : ImVec2(100, -1)), total_bb.Max + ImVec2(-10, 1));
    ItemSize(total_bb, style.FramePadding.y);
    ItemAdd(total_bb, id);
    ...
    it_anim->second.combo_size = ImLerp(it_anim->second.combo_size, it_anim->second.opened_combo ? value * 26 + 13 : 0.f, anim_speed);
    ...
    //window->DrawList->AddRectFilled(rect_bb.Min, rect_bb.Max, GetColorU32(background_color), style.FrameRounding);
    ...
    PushFont(semibold_font);
    window->DrawList->AddText(ImVec2(total_bb.Max.x - CalcTextSize(preview_value).x, center_text(rect_bb.Min, rect_bb.Max, preview_value).y), GetColorU32(it_anim->second.text_color), preview_value);
    PopFont();
    ...
    ImGui::SetNextWindowSize(ImVec2(rect_bb.GetWidth() + 10, it_anim->second.combo_size));

    PushStyleVar(ImGuiStyleVar_WindowRounding, style.FrameRounding);
    PushStyleColor(ImGuiCol_WindowBg, ImColorToImVec4(background_color));
...
bool ImGui::NewBeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags, bool hide_lable)
{
    ...
    it_anim->second.text = ImLerp(it_anim->second.text, popup_open ? text_color[0] : hovered ? text_color[0] : text_color[1], g.IO.DeltaTime * 16.f);
    it_anim->second.combo = ImLerp(it_anim->second.combo, popup_open ? background_color : hovered ? background_color : background_color, g.IO.DeltaTime * 16.f);

    const float value_x2 = ImMax(bb.Min.x, bb.Max.x - arrow_size);

    // GetWindowDrawList()->AddRectFilled(bb.Min, ImVec2(value_x2, bb.Max.y), GetColorU32(bg_color), 10.f, (flags & ImGuiComboFlags_NoArrowButton) ? ImDrawFlags_RoundCornersAll : ImDrawFlags_RoundCornersLeft);

    // GetWindowDrawList()->AddRectFilled(ImVec2(value_x2, bb.Min.y), bb.Max, GetColorU32(bg_color), 10.f, (w <= arrow_size) ? ImDrawFlags_RoundCornersAll : ImDrawFlags_RoundCornersRight);

    // GetWindowDrawList()->AddRect(bb.Min, ImVec2(bb.Min.x + (w - 100), bb.Max.y), GetColorU32(bg_color), 10.f);

    PushStyleColor(ImGuiCol_Text, GetColorU32(text_color[0]));
...
bool ImGui::NewBeginComboPopup(ImGuiID popup_id, const ImRect& bb, ImGuiComboFlags flags)
{
    ...
    PushStyleVar(ImGuiStyleVar_WindowPadding, ImVec2(15, 5));
    PushStyleVar(ImGuiStyleVar_PopupRounding, 4.f);

    PushStyleColor(ImGuiCol_PopupBg, ImColorToImVec4(background_color));
    PushStyleColor(ImGuiCol_Border, ImColorToImVec4(background_color));
...
struct slider_state {
    ...
    float text_offset;
    bool value_changing;
    float circle_offset[20];
    ImVec4 value_frame_col;
};

void DrawValueRect(ImVec2 pos, const char* value, ImColor frame_col, ImColor text_col)
{
    ImGuiWindow* window = ImGui::GetCurrentWindow();
    if (window->SkipItems)
        return;

    static float value_size;

    ImGui::PushFont(medium_font);

    value_size = ImLerp(value_size, ImGui::CalcTextSize(value).x / 2, anim_speed);

    ImRect frame_bb(pos - ImVec2(5 + value_size, -12), pos + ImVec2(5 + value_size, 27));

    ImGui::GetForegroundDrawList()->AddTriangleFilled(ImVec2(pos.x + 3, frame_bb.Min.y), ImVec2(pos.x - 3, frame_bb.Min.y), ImVec2(pos.x, frame_bb.Min.y - 5.f), frame_col);

    ImGui::GetForegroundDrawList()->AddRectFilled(frame_bb.Min, frame_bb.Max, frame_col, 3.f);

    ImGui::GetForegroundDrawList()->AddText(center_text(frame_bb.Min, frame_bb.Max, value), text_col, value);
    ImGui::PopFont();
}
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    // Slider behavior
    ImRect slider_bb(frame_bb.Min + ImVec2(100, 6), ImVec2(frame_bb.Max.x, frame_bb.Max.y - 6));
    ...
    it_anim->second.grab_offset = ImLerp(it_anim->second.grab_offset, grab_bb.Min.x + (grab_bb.Max.x - grab_bb.Min.x) / 2 - frame_bb.Min.x, anim_speed * 3);
    it_anim->second.text_color = ImLerp(it_anim->second.text_color, text_color[0], anim_speed);
    it_anim->second.value_color = ImLerp(it_anim->second.value_color, it_anim->second.value_changing || hovered ? ImColor(1.f, 1.f, 1.f, 1.f) : ImColor(1.f, 1.f, 1.f, 0.f), anim_speed);
    it_anim->second.value_frame_col = ImLerp(it_anim->second.value_frame_col, it_anim->second.value_changing || hovered ? main_color : ImColor(main_color.x, main_color.y, main_color.z, 0.f).Value, anim_speed);

    window->DrawList->AddRectFilled(slider_bb.Min, slider_bb.Max, GetColorU32(background_color), style.FrameRounding);

    for (int i = 1; i < 20; i++) {
        it_anim->second.circle_offset[i] = ImLerp(it_anim->second.circle_offset[i], it_anim->second.grab_offset - 1, anim_speed / (i / 10.f));

        window->DrawList->AddCircleFilled(ImVec2(frame_bb.Min.x + it_anim->second.circle_offset[i], slider_bb.Max.y - 2), 6.f - (i / 5.f), GetColorU32(main_color), 360);
    }

    PushClipRect(frame_bb.Min + ImVec2(100, 0), frame_bb.Max + ImVec2(10, 0), true);
    window->DrawList->AddRectFilled(slider_bb.Min, ImVec2(frame_bb.Min.x + it_anim->second.grab_offset, slider_bb.Max.y), GetColorU32(main_color), style.FrameRounding);
    PopClipRect();
    ...
    DrawValueRect(ImVec2(frame_bb.Min.x + it_anim->second.grab_offset - 1, slider_bb.Max.y - 2), value_buf, GetColorU32(it_anim->second.value_frame_col), GetColorU32(it_anim->second.value_color));
...
bool ImGui::TempInputText(const ImRect& bb, ImGuiID id, const char* label, char* buf, int buf_size, ImGuiInputTextFlags flags)
{
    ...
    bool value_changed = MyInputTextEx(label, NULL, buf, buf_size, bb.GetSize(), flags | ImGuiInputTextFlags_MergedItem);
    if (init)
    {
...
bool ImGui::MyInputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, bool show_lable, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    it_anim->second.background_color = ImLerp(it_anim->second.background_color, winbg_color, anim_speed);
    it_anim->second.main_color = ImLerp(it_anim->second.main_color, tab_color, anim_speed);
    it_anim->second.text_color = ImLerp(it_anim->second.text_color, bg_color, anim_speed);
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
    {
        ...
        ImGui::SetCursorScreenPos(total_bb.Max - ImVec2(16, 16));
        ...
        PushStyleColor(ImGuiCol_PopupBg, ImColorToImVec4(child_col[0]));
        PushStyleColor(ImGuiCol_Border, ImColorToImVec4(child_col[1]));
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    it_anim->second.text_color = ImLerp(it_anim->second.text_color, selected ? ImColor(1.f, 1.f, 1.f, 1.f).Value : text_color[1], anim_speed);
    it_anim->second.frame_color = ImLerp(it_anim->second.frame_color, selected ? main_color : ImColor(main_color.x, main_color.y, main_color.z, 0.f).Value, anim_speed);
